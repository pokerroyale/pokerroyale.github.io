	...
	points[5] = 35;/*30*/	multi[5] = 3;/*4*/	inc_points[5] = 25;		inc_multi[5] = 2;	// 30x3
	points[6] = 35;		multi[6] = 4;		inc_points[6] = 25;		inc_multi[6] = 2;	// 15x2
	points[7] = 40;		multi[7] = 4;		inc_points[7] = 30;		inc_multi[7] = 2;	// 25x2
	points[8] = 60;		multi[8] = 6;/*7*/	inc_points[8] = 30;		inc_multi[8] = 3;
	points[9] = 80;/*100*/	multi[9] = 8;		inc_points[9] = 40;		inc_multi[9] = 3;	// 40x4
	points[10] = 100;/*120*/multi[10] = 10;/*12*/	inc_points[10] = 40;	inc_multi[10] = 4;	// 35x3
	points[11] = 120;/*140*/multi[11] = 12;/*14*/	inc_points[11] = 40;	inc_multi[11] = 5;	// 40x4
	points[12] = 140;/*160*/multi[12] = 14;/*16*/	inc_points[12] = 50;	inc_multi[12] = 5;	// 50x3
	...
	function delete_game(){
		...
		Cookies.remove("PokerRoyale_plays");
	...
	function load_game(){
		if(Cookies.get("PokerRoyale_level")){
			level = Cookies.get("PokerRoyale_level").split("_").map(Number);
			plays = Cookies.get("PokerRoyale_plays").split("_").map(Number);
			if(Cookies.get("PokerRoyale_current_play")) current_play = Cookies.get("PokerRoyale_current_play").split("_").map(Number);
			else current_play = new Array();
			if(Cookies.get("PokerRoyale_current_score")) current_score = Cookies.get("PokerRoyale_current_score").split("_").map(Number);
			else current_score = new Array();
			total_score = parseInt(Cookies.get("PokerRoyale_total_score"));
			max_plays = parseInt(Cookies.get("PokerRoyale_max_plays"));
			plays_left = parseInt(Cookies.get("PokerRoyale_plays_left"));
	...
	function change_max_discards(inc){
		clearTimeout(timeout);
		if(!is_active($("#levels #max_discards"))){
			refresh();
	...
	function change_cards(inc){
		clearTimeout(timeout);
		if((!is_active($("#game_footer #cards"))) || (!inc)){
			refresh();
	...
	function change_discards(inc){
		clearTimeout(timeout);
		if((!is_active($("#game_footer #discards_left"))) || (!inc)){
			refresh();
			activate($("#game_footer #discards_left"));
	...
	function change_plays(inc){
		clearTimeout(timeout);
		if((!is_active($("#game_footer #plays_left"))) || (!inc)){
			refresh();
			activate($("#game_footer #plays_left"));
	...
	function get_label(h, abbr = false, warning = false){
		...
		switch(h){
			...
			case 9: label = (abbr) ? "Esc. Color/Real" : "Escalera de Color / Real"; if(warning) label = "una <span>" + label + "</span> "; break;
